        ...
        .on("readable", () => {
          for (let row = parser.read(); row; row = parser.read()) {
            ...
            if (country.trim().toUpperCase() == tempCountry.trim().toUpperCase() ) {
              currencyMeta = { CurrencyName: JSON.parse(JSON.stringify(row)).currencyName,  CurrencyCode:  JSON.parse(JSON.stringify(row)).currencyCode};
              break;
            }
          }
...
function getCountryAndCurrencyCode(currencyCode) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (currencyCode !== null && typeof(currencyCode) != 'undefined' && currencyCode.trim().length == 3) {
        var currencyMeta;
        fileStream
          .pipe(parser)
          .on("error", error => console.error(error))
          .on("readable", () => {
            for (let row = parser.read(); row; row = parser.read()) {
              ...
              if (currencyCode.trim().toUpperCase() == tempCurrencyCode.trim().toUpperCase() ) {
                currencyMeta = { CurrencyName: JSON.parse(JSON.stringify(row)).currencyName,  Country:  JSON.parse(JSON.stringify(row)).country};
                break;
              }
            }
